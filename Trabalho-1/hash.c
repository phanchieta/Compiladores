...
int add_string(hash_table_t *hashtable, char *str, char *str2)
{
    list_t *new_list;
    list_t *current_list;
    unsigned int hashval = hash(hashtable, str);
    ...
    if (str2!=NULL)
    new_list->string2 = strdup(str2);

    else new_list->string2=NULL;
...
{
    hash_table_t *hashtable=create_hash_table(17); /*para adicionar mais valores, aumentar o numero e inserir ao final*/

    add_string(hashtable, "program", PROGRAM );
    add_string(hashtable, "begin", BEGIN );
